<?php
 ...
 $requette = "SELECT COUNT(*) FROM UTILISATEUR WHERE EMAIL = :email";
 $rowCount = $conn->prepare($requette);

 $userRequet = "SELECT ID_UTILISATEUR FROM UTILISATEUR WHERE EMAIL = :email";
 $getUserId = $conn->prepare($userRequet);
 ...
 $reserve = "SELECT EVENEMENT.DESCRIPTION, EVENEMENT.TITRE, TARIF_NORMAL, TARIF_REDUIT , DATE , CAPACITE - COUNT(NUM_BILLET) AS 'DISPONIBLE' FROM BILLET INNER JOIN FACTURE USING(NUM_FACTURE) RIGHT JOIN VERSION USING(ID_VERSION) INNER JOIN EVENEMENT USING(ID_EVENT) INNER JOIN SALLE USING (NUM_SALLE) WHERE ID_VERSION = :ID;";
 $details = $conn->prepare($reserve);

 $achats = "SELECT ID_UTILISATEUR, NOM, PRENOM,EMAIL, DATE_ACHAT, NUM_FACTURE AS NF, TITRE, TYPE, TARIF_REDUIT, TARIF_NORMAL,
 ...
 $profile = "SELECT  NOM , PRENOM, ID_UTILISATEUR, EMAIL, SIGNUPDATE, USERNAME, ifnull(MAX(NUM_FACTURE),'NO FACTURES') AS RECENT,PASSWORD,COUNT(NUM_FACTURE) FROM FACTURE RIGHT JOIN UTILISATEUR USING(ID_UTILISATEUR) WHERE ID_UTILISATEUR = :user;";
 $profilePrepare = $conn->prepare($profile);
?>